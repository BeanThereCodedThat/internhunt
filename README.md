...
`HttpClient` - nothing here drives a headless browser. Selenium was tried
early for Unstop and dropped (see git history) in favor of hitting Unstop's
internal API directly; the unused base class and the Selenium/WebDriverManager
dependencies it needed have since been removed from the project entirely.
