...
// ─── Skill matching (Phase 2 — offline keyword extractor + scoring) ───────────

// Batch match scores for job-card badges. Returns { [jobId]: score }; score -1 = no data yet.
export async function fetchMatchScores(jobIds, userId) {
  if (!jobIds || jobIds.length === 0 || !userId) return {}
  return api.get(`/match/${userId}/scores?jobs=${jobIds.join(',')}`).catch(() => ({}))
}
